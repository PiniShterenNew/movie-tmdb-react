...
import { useFavoritesStore, type MediaType } from "@/shared/store/favorites.store";
...
export const FavoritesPage: React.FC = () => {
  const { items, removeFavorite, clearAll, exportData, importData } = useFavoritesStore();
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "moviehub-favorites.json";
    a.click();
    ...
    <div className="container mx-auto px-4 py-8 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-2">
            ...
            {language === "he" ? "המועדפים שלי" : "My Favorites"}
          </h1>
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-muted-foreground">
            {language === "he"
              ? "הוסף סרטים וסדרות שאתה אוהב"
              : "Add movies and TV shows you love"}
          </p>
        </div>
      ) : viewMode === "grid" ? (
...
const FavoriteCard: React.FC<FavoriteItemProps> = ({
  item,
  onRemove,
  onClick,
  language,
  ...
  return (
    ...
    >
      ...
      <Button
        variant="destructive"
        size="icon"
        className="absolute top-10 left-2 h-7 w-7 opacity-0 group-hover:opacity-100 transition-opacity"
        onClick={(e) => {
          e.stopPropagation();
          onRemove();
        }}
...
const FavoriteListItem: React.FC<FavoriteItemProps> = ({
  item,
  onRemove,
  onClick,
  language,
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <button onClick={onClick} className="text-right block">
          <h3 className="font-medium text-foreground group-hover:text-primary transition-colors truncate flex items-center gap-2">
